...
public final class ControlUnitCommunicationImpl extends Thread implements ControlUnitCommunication {
    ...
    private void httpRequest() {
        ...
        requestString.append(SEPARATOR + this.controller.getOpeningFromDashboard());
    ...
    private void parseResponse(final String message) {
        if (message.startsWith(RECEIVE_MESSAGE_START)) {
            ...
            this.controller.setTemperatureState(TemperatureState.valueOf(temperature[1].toUpperCase()));
            ...
            if (!lines[1].isEmpty()) {
                ...
                List<Pair<Long, Float>> list = new ArrayList<>();
                for (String string : history) {
                    string = string.substring(string.indexOf(PAIR_START) + 1, string.indexOf(PAIR_END));
                    final var pair = string.split(PAIR_SEPARATOR);
                    list.add(new Pair<Long,Float>(Long.parseLong(pair[0]), Float.parseFloat(pair[1])));
                }
            ...
            this.controller.setWindowMode(WindowMode.valueOf(opening[0].toUpperCase()));
